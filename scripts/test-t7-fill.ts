...
async function main() {
  ...
  console.log(
    ...
    JSON.stringify(fieldMap.total_riser_height_feet_and_inches),
    "| diameter_feet_only =",
    JSON.stringify(fieldMap.diameter_feet_only),
  );
  ...
  const remaining = check.getForm().getFields().length;
  console.log("fields remaining after flatten:", remaining);

  writeFileSync("test-t7-filled.pdf", flat);
  console.log("wrote test-t7-filled.pdf");
}
